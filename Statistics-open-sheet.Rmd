...
For low values of $\lambda$, our Ridge regression constrains very little, and at $\lambda = 0$, we just run MLE, where we have low bias, but have higher variance. At $\lambda$ at very high values, we drive all our $\beta$ estimates to 0, with higher variance, but low variance. We want to find the $\lambda$ which minimises MSE (not just RSS or high $R^2$, as these measure how well our model fits the training data, but do not guarantee a good fit for our test data, so it may no predict well new observations).
...
### Leverage
The basic tool to examine the fit are the residuals, which are not independent and sum to 0 if there is the intercept. Their covariance matrix is $var(e)=\sigma^2[I-H]$. The leverage of an observation point $i$ is $h_{ii}$ of the $H=X(X^TX)^{-1}H$ matrix. Since the trace of $H$ is $p$ (the dimension of the model space), we say that $h_ii$ is large if it is three times greater than the average. So if $h_ii\geq \frac{3(M+1)}{N}$.
...
par(new=TRUE)
curve(0.5+0*x,from=-3,to=3,col="red")
